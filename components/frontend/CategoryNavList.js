    ...
    return (
        <nav className="flex gap-8 h-full max-lg:hidden max-lg:order-3">
            {Object.keys(categories).map((item) => (
                <div
                    key={item}
                    className="uppercase h-16 flex items-center hover:border-b-black hover:border-b  hover:cursor-pointer"
                    onMouseEnter={() => setActiveMenu(item)}
                    onMouseLeave={() => setActiveMenu(null)}
                >
                    <Link href={`/categories/${item.toLowerCase()}`} className='h-full text-center flex items-center'>{item}</Link>
                    {/* DROPDOWN */}
                    {activeMenu === item && (
                        <div className="absolute left-0 top-full w-full border  " onMouseEnter={() => setActiveMenu(item)}
                            onMouseLeave={() => setActiveMenu(null)}>
