...
const Navbar = () => {
    ...
    return (
        <header className="border-b text-primary bg-terteiary top-0 z-50 absolute w-full">
                    ...
                    <nav className="flex gap-8 h-full ">
                        {Object.keys(menuData).map((item) => (
                            ...
                            >
                                ...
                                {/* DROPDOWN */}
                                {activeMenu === item && (
                                    <div className="absolute left-0 top-full w-full bg-white/20 shadow-xl px-4 " onMouseEnter={() => setActiveMenu(item)}
                                        onMouseLeave={() => setActiveMenu(null)}>
                                        <ul className="flex gap-4 justify-center">
                                            {menuData[item].map(({ name, icon: Icon }) => (
                                                <li
                                                    key={name}
                                                    className="flex flex-col items-center gap-3 px-6 py-6 cursor-pointer hover:bg-primary/60
                          hover:text-primary/60  transition text-primary"
                                                >
