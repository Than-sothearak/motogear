...
const Navbar = () => {
    ...
    return (
        <header className="border-b bg-white/80 bg-blur-lg top-0 z-50 relative sticky">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <nav className="flex gap-8 h-full ">
                        {Object.keys(menuData).map((item) => (
                            <div
                                key={item}
                                className=" h-14 font-bold flex items-center hover:border-b-black hover:border-b  hover:cursor-pointer"
                            ...
                            >
                                ...
                                {/* DROPDOWN */}
                                {activeMenu === item && (
                                    <div className="absolute left-0 top-full w-full bg-white shadow-xl p-4 border" onMouseEnter={() => setActiveMenu(item)}
                                        onMouseLeave={() => setActiveMenu(null)}>
                                        <ul className="flex gap-4 justify-center">
                                            {menuData[item].map(({ name, icon: Icon }) => (
                                                <li
                                                    key={name}
                                                    className="flex flex-col items-center gap-3 px-3 py-2 rounded-lg cursor-pointer
                          hover:bg-gray-100 transition text-gray-700 hover:text-black"
                                                >
                                                    <Icon size={25} className="text-gray-500" />
                                                    <span>{name}</span>
                                                </li>
                                            ))}
                                        </ul>
