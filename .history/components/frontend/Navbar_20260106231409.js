        ...
        <div className="flex items-center justify-between">
          ...
          <nav className="flex gap-8 h-full ">
            {Object.keys(menuData).map((item) => (
              <div
                key={item}
                className=" h-14 flex items-center hover:border-b-black hover:cursor-pointer"
                onMouseEnter={() => setActiveMenu(item)}
                onMouseLeave={() => setActiveMenu(null)}
              >
                <div className="h-full flex flex-col justify-between">
                    <div className="h-full flex items-center"> {item}<div className="hover:border-b border-black"></div></div>
                ...
                {/* DROPDOWN */}
                {activeMenu === item && (
                  <div className="absolute left-0 top-full w-full bg-white shadow-xl p-4 border">
