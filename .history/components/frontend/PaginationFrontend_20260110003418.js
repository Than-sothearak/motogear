...
const PaginationFrontend = ({ pathname, totalPages, currentPage, query,ITEM_PER_PAGE }) => {
    const searchParams = useSearchParams()
    const sortKey = searchParams.get("sortKey");
    const sortDirection = searchParams.get("sortDirection");
    const limit = searchParams.get("limit") || ITEM_PER_PAGE
  ...
  return (
    <div className="w-full flex justify-center items-center gap-2 mb-10 mt-4">
      ...
      {pageNum > 1 ? (
        ...
        <div className="bg-primary border opacity-50 cursor-not-allowed px-4 py-1 rounded-md hover:bg-primary hover:text-primarytext">
          Prev
        </div>
      )}
      ...
      {pageNum < totalPages ? (
        ...
        <div className="bg-primary text-primarytext border opacity-50 cursor-not-allowed px-4 py-1 rounded-md">
          Next
        </div>
      )}
    </div>
