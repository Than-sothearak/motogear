...
export const OrderTable = ({ initialOrders,initialHasMore, page }) => {
    ...
    // Call server action
    const {result} = await getOrders(nextPage);
    ...
    return (
      ...
      {hasMore && (
        <button
          onClick={loadMore}
          className="mt-4 px-4 py-2 border text-primarytext t rounded "
        >
    how More
        </button>
      )}
