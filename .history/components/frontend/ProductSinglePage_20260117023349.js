...
export default function ProductSinglePage({ product, session }) {
  ...
  const handleClick = (product, color, size,  mainImage ) => {
    addProduct(product, color, size,  mainImage);
  };
    ...
    <div className="container mx-auto lg:flex lg:gap-8 my-10 lg:px-0 px-4">
      ...
      <div className="lg:w-1/2 flex flex-col">
              ...
              {availableColors.map((color) => {
                ...
                return (
                  <button
                    ...
                    className={`relative px-4 py-2 border rounded-md transition-all ${selectedColor === color
              ...
              {availableSizes.map((size) => {
                ...
                return (
                  <button
                    ...
                    className={`relative px-4 py-2 border rounded-md transition-all ${selectedSize === size
                      ? "bg-tertiary text-primary"
                      : "bg-primary text-primarytext"
                      } ${!hasStock
                        ? "opacity-100 cursor-not-allowed"
        ...
        <div className="mt-6 flex items-center gap-4">
          ...
          <button
            onClick={() => handleClick(product, selectedColor, selectedSize, 1, mainImage, basePrice)}
            className={`bg-tertiary text-primary px-6 py-2 border rounded-md font-semibold hover:text-primary transition-all ${stock === 0 ? "opacity-50 cursor-not-allowed" : ""
              }`}
            disabled={stock === 0}
          >
            Add to Cart {mainImage}
          </button>
        </div>
        {/* Description */}
        <div className="mt-8">
