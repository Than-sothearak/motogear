...
const UserProfileForm = ({ user }) => {
    ...
    useEffect(() => {
        if (states?.success) {
            const notify = () => toast.success(states.message);
            setIsEdit(false)
            setState(states)
            notify();

        } else if (states?.errors) {
            const notify = () => toast.error(states.message);
            setState(states)
            notify();
    ...
    return (
        <div className="bg-white rounded-sm shadow p-6 lg:w-1/2 h-full w-full border">
            ...
            <button
                ...
                className="px-4 py-2 text-sm rounded-sm border hover:bg-gray-100 mt-2"
            >
