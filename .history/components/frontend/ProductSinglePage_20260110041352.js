        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          ...
          {variants?.images && (
            variants.map((v, idx) => (
              <div
                key={idx}
                className={`relative  w-20 h-20 border rounded-md overflow-hidden cursor-pointer ${mainImage === v.images ? "ring-2 ring-primary" : ""
                  }`}
                onClick={() => setMainImage(v.images)}
              >
                <Image src={v.images} alt={`${productName} ${idx}`} fill className="object-contain" />
              </div>
            ))
