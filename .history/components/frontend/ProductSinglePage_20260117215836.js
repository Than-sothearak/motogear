...
export default function ProductSinglePage({ product, session }) {
  ...
  const handleClick = (product, color, size, qty, mainImage ) => {
    addProduct(product, color, size, qty, mainImage);
  };
    ...
    <div className="container mx-auto lg:flex lg:gap-8 my-10 lg:px-0 px-4">
      ...
      <div className="lg:w-1/2 flex flex-col justify-start items-center gap-4 mb-10 relative">
        ...
        <div className="flex gap-2 pb-2 max-w-full overflow-x-auto">
          {imageUrls.map((img, idx) => (
            <div
              key={`main-${idx}`}
              className={`relative w-20 h-20 border rounded-md cursor-pointer shrink-0 ${mainImage === img ? "ring-2 ring-primary" : ""
                }`}
              onClick={() => {
                setMainImage(img);
                setCurrentIndex(idx);
      ...
      <div className="lg:w-1/2 flex flex-col">
              ...
              {availableColors.map((color) => {
                ...
                return (
                  <button
                    ...
                    className={`relative px-4 py-4 border rounded-full transition-all ${selectedColor === color
                      ? "bg-"+color.toLowerCase()+"-500"+ "text-primary bg-green-500"
                      : "bg-"+color.toLowerCase()+"-500"+" text-primary"
                      } ${!hasStock
              ...
              {availableSizes.map((size) => {
                ...
                return (
                  <button
                    ...
                    className={`relative px-4 py-2 border rounded-sm transition-all ${selectedSize === size
        ...
        <div className="mt-6 flex items-center gap-4">
          ...
          <button
            onClick={() => handleClick(product, selectedColor, selectedSize, 1, mainImage)}
            className={`bg-tertiary text-primary px-6 py-2 border rounded-sm font-semibold hover:text-primary transition-all ${stock === 0 || !selectedColor || !selectedSize ? "opacity-50 cursor-not-allowed" : ""
              }`}
            disabled={stock === 0 || !selectedColor || !selectedSize}
          >
            Add to Cart
          </button>
        </div>
