import { formatDate, formatDateForForm, formatDateTime } from '@/utils/formatDate';
        ...
        <div className=" bg-white rounded-xl shadow p-6 w-full max-h-full">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {orders.map((order) => (
                            <tr key={order._id} className="border-b last:border-0 text-left">
                                <td className="py-3 font-medium text-left">#{order._id}</td>
                                <td className='text-left'>{formatDateTime(order.
                                    updatedAt)}</td>
