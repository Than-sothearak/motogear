...
<div className="absolute inset-0 flex items-center justify-center text-center px-4">
  <div>
    {/* h1 Behind Image */}
    <h1 className={`lg:text-8xl text-6xl font-extrabold text-primarytext mb-4 uppercase z-0 absolute top-16 right-0 w-full ${myFont.className}`}>
      {slides[0].title}
    </h1>
  ...
  {[...slides, slides[0]].map((slide, i) => (
    <div key={i} className="relative min-w-full h-screen">
      <Image
        ...
        alt={slide.title}
        fill
        className="z-10 object-cover max-md:object-scale-down object-center"
        priority={i === 0}
      />
