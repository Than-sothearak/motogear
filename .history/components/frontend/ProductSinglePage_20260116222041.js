...
export default function ProductSinglePage({ product, session }) {
  ...
  } = useProductVariants(variants, imageUrls);
        ...
        <div className="flex gap-2 pb-2 max-w-full overflow-x-auto ">
          ...
          {variants
            ...
            .map((v, idx) => (
              ...
              >
                <Image src={v.images} alt={`${productName} variant ${idx}`} fill className="object-contain" />
              </div>
            ))}
        </div>
                ...
                return (
                  <button
                    ...
                    className={`relative px-4 py-2 border rounded-md transition-all
                      ${selectedColor === color ? "bg-tertiary text-primary" : "bg-primary text-primarytext"}
                      ${!hasStock ? "opacity-100 cursor-not-allowed" : "hover:border-tertiary"}
                    `}
                  >
                    {color}
                    {!hasStock && (
              ...
              {availableSizes.map(size => {
                ...
                return (
                  <button
                    ...
                    className={`relative px-4 py-2 border rounded-md transition-all
                      ${selectedSize === size ? "bg-tertiary text-primary" : "bg-primary text-primarytext"}
                      ${!hasStock ? "opacity-100 cursor-not-allowed" : "hover:border-tertiary"}
                    `}
                  >
                    {size}
