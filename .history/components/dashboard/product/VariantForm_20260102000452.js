...
import ChooseImageFile from '../ChooseImageFiles';
    ...
    function removeVariantImage(variantIndex, imageIndex) {
        setVariants((prev) =>
            prev.map((variant, i) => {
                ...
                URL.revokeObjectURL(variant.images[imageIndex].preview); // free memory
        ...
        <div className="flex gap-4 max-2xl:flex-col">
            ...
            <div className="border rounded-md py-4 w-full">
                    ...
                    {variants?.map((variant, index) => (
                        <div key={index} className='flex-col flex gap-4'>
                            ...
                            <ChooseImageFile
                                ...
                                onChange={(newImages) => {
                                    setVariants((prev) =>
                                        prev.map((v, i) => (i === index ? { ...v, images: newImages } : v))
                                    );
                                }}
                                onRemove={(imageIndex) => removeVariantImage(index, imageIndex)}
                                inputId={`fileInput-${index}`} // unique per variant
                            />
                        </div>
                ...
                <div className='flex gap-4'>
                        ...
                        onClick={addVariants}
                        type="button"
                        className=" bg-tertiary hover:bg-tertiary/80 text-white px-4 py-2 rounded-lg"
                    >
                        Add Variant
                    </button>
