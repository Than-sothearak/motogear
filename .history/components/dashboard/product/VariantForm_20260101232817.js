...
function removeVariantImage(variantIndex, imageIndex) {
    ...
    // reset the input so the removed file is no longer selected
    const fileInput = document.getElementById(`variantInput-${variantIndex}`);
    ...
    value={JSON.stringify(
        variants.map((v) => ({
            ...
            price: v.price,
            sku: v.sku,
            imageCount: v.images.length, // store how many images to match files
        }))
    )}
    readOnly
/>
