...
const VariantForm = ({ productData, onDataChange, setVariants, setProperties, properties, variants, giveErrors
 }) => {

 
const [errors, setErrors] = useState(giveErrors || {});
...
const validate = () => {
  ...
  variants.forEach((v, idx) => {
    ...
    if (!v.stock) newErrors[`variant-${idx}-stock`] = "Stock is required";
    if (!v.price) newErrors[`variant-${idx}-price`] = "Price is required";
  });

  properties.forEach((p, idx) => {
    if (!p.name) newErrors[`property-${idx}-name`] = "Name is required";
    if (!p.part) newErrors[`property-${idx}-part`] = "Value is required";
  });
      ...
      <div className="border rounded-md py-4 w-full">
        ...
        <div className="px-3 space-y-2 text-sm">
          {variants.map((variant, index) => (
            ...
            <input
  className={`p-2 border rounded-lg w-full ${
    errors?.[`variant-${index}-size`] ? "border-red-500" : ""
  }`}
  placeholder="Size"
  value={variant.size}
  onChange={(e) => {
...
{errors?.[`variant-${index}-size`] && (
  <p className="text-red-500 text-xs">{errors[`variant-${index}-size`]}</p>
)}
                <input
