...
export default function ProductTable({ products, categories }) {
  ...
  const handleChange = (e) => {
    ...
    if (selectedSlug) {
      params.set("category", selectedSlug); // set new category
    } else {
      params.delete("category"); // optional: remove category if empty
    }

    router.push(`/dashboard/products?page=1${params.toString()}`); // keep status param
  };
