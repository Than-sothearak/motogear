...
function ProductForm({ productData, categories }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      const notify = () => toast.error("Somting wrong");
      notify();
      setIsPeding(false)
              ...
              <div className="flex gap-2 items-center">
                {!isEditing ? (
                  <button
                    ...
                    className="border text-primaryText px-6 py-2 rounded-md hover:bg-tertiary hover:text-primary"
                  >
                    Edit
                  </button>
                ) : (
                  <button
                    ...
                    className="bg-tertiary text-white px-6 py-2 rounded-md h-full hover:bg-tertiary/80"
                  >
                    Cancel
                  </button>
                )}
                ...
                {isEditing && (
                  <div className="flex justify-end">
                    <button
                      type="submit"
                      disabled={isPending}
                      className={`bg-tertiary text-white px-6 py-2 rounded-md hover:bg-tertiary/80 ${isPending ? 'opacity-50' : 'opacity-100'}`}
                    >
                      {isPending ? 'Saving...' : productData ? "Update Product" : "Save Product"}
                    </button>
                  </div>
                )}
              </div>
            ...
            {isEditing && (
              <div className="flex justify-end mt-4">
                <button
                  type="submit"
                  disabled={isPending}
                  className={`bg-tertiary text-white px-6 py-2 rounded-lg hover:bg-tertiary/80 ${isPending ? 'opacity-50' : 'opacity-100'}`}
                >
                  {isPending ? 'Saving...' : productData ? "Update Product" : "Save Product"}
                </button>
              </div>
