...
const AddPropertyForm = ({ formData, setFormData }) => {
  ...
  return (
    <div className="flex flex-col gap-2 border p-4 rounded-md bg-slate-100 text-secondarytext">
      <button
        type="button"
        className={`${anyPropertyEmpty
            ? "cursor-not-allowed bg-primary"
            : "hover:bg-secondary bg-tertiary"
          } w-36 p-2 text-primarytext rounded-md flex items-center gap-2 text-base`}
        onClick={addPart}
