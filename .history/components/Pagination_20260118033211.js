...
const Pagination = ({ pathname, totalPages, currentPage, query }) => {
  ...
  const getHref = (page) => {
    ...
    params.set("page", page);             // always use this page
    params.set("sortKey", sortKey);       // keep sortKey
    params.set("sortDirection", sortDirection); // keep sortDirection
    if (query) params.set("query", query); // add query if exists
    else params.delete("query");           // remove query if none

    return `/dashboard/${pathname}?${params.toString()}`;
  };
  return (
