...
const getHref = (page) => {
  // If params has a "page", use it; otherwise use the function argument
  const finalPage = params.has("page") ? params.get("page") : page;

  // Build the href
  return `/dashboard/${pathname}?page=${finalPage}${
    query ? `&query=${query}` : ""
  }&sortKey=${sortKey || ""}&sortDirection=${sortDirection || "descending"}&${params.toString()}`;
};
