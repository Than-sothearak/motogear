...
const SideBarMain = ({ handleClick, isOpen, session, currentUser, navigation,servicesCount,link }) => {
  ...
  return (
    <div
      className={`bg-primary text-primarytext h-full ${isCollapsed ? 'p-0' : 'p-2'} h-full overflow-y-auto transition-all duration-300 ease-in-out
        max-lg:fixed top-0 left-0 z-50
        ${isOpen ? "translate-x-0" : "-translate-x-full"}
        ${isCollapsed ? "w-14 justify-start flex-col items-center flex " : "w-80 max-md:mt-[118px]" } 
        lg:translate-x-0  `}
    >
      {/* Top Section */}
