...
export async function getProducts(slug, catParams, query, page, status) {
  await mongoDb();
  let queryData = {}
  if (status) {
  ...
  const selectedCatSlug = catParams?.cat || 'all'
  const { parent, childCategories } = await getAllCategoryBySlug(slug[0])
