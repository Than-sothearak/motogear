...
export async function updateClientProfile(userId, prevState, formData) {
  ...
  try {
    ...
    if (!address) errors.address = "Address is required";
    if (Object.keys(errors).length > 0) {
      return { errors, success: false , message: "Someting wrong!"};
    }
   ...
   revalidatePath('/dashbaord')
    ...
    console.log("Error updating user:", err , { message: err});
  }
