...
export default async function cartPage({ searchParams }) {
  ...
  const session = await stripe.checkout.sessions.retrieve(success, {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
     ...
     <div className="flex justify-between">
       ...
       <Link href={`/dashboard`} className=" underline">View order</Link>
     </div>
      ...
      <ButtonShopNow link={`/products`} title={`explore more`}/>
    </div>
  );
}
