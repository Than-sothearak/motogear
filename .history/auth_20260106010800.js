  ...
  providers: [
    ...
    Credentials({
      ...
      async authorize(credentials) {
        ...
        // Logic to prevent the "[object Object]" error
        if (typeof credentials.telegramData === 'string') {
          try {
            telegramData = JSON.parse(credentials.telegramData);
          } catch (e) {
            // If parsing fails because it was already an object stringified poorly
            console.error("JSON Parse failed, trying raw credentials", e);
            telegramData = credentials.telegramData;
          }
        } else {
          telegramData = credentials.telegramData;
        }
        ...
        telegramData = JSON.parse(credentials);
